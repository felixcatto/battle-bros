...
export const batter = (options) => {
  ...
  const newHp = hp - Math.max(APRDmg + Math.floor(armorOverkill * headDmgModifier), 10);
...
export const batterNimble = (options) => {
  ...
  const dmgToHp = Math.max(
    APRDmg + Math.floor(armorOverkill * headDmgModifier * nimbleDmgReduction),
    Math.floor(10 * nimbleDmgReduction),
  );
